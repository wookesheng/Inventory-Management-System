<?php
session_start();
require_once "config.php";

// Check if user is in the verification process
if(!isset($_SESSION['reset_user_id']) || !isset($_SESSION['reset_email']) || !isset($_SESSION['reset_code'])) {
    header("location: forgot_password.php");
    exit;
}

if(isset($_POST["verify"])) {
    $code = trim($_POST["code"]);
    
    // Validate code format
    if(!preg_match("/^\d{6}$/", $code)) {
        $error = "Please enter a valid 6-digit code.";
    } else {
        // Get current user data
        $sql = "SELECT reset_token, reset_expiry FROM users WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "i", $_SESSION['reset_user_id']);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        $user = mysqli_fetch_assoc($result);
        mysqli_stmt_close($stmt);

        $current_time = date('Y-m-d H:i:s');
        
        // Log verification attempt details
        error_log("Verification attempt:\n" .
                 "User ID: {$_SESSION['reset_user_id']}\n" .
                 "Entered Code: $code\n" .
                 "Expected Code: {$_SESSION['reset_code']}\n" .
                 "Stored Token: {$user['reset_token']}\n" .
                 "Expiry Time: {$user['reset_expiry']}\n" .
                 "Current Time: $current_time");

        // Check if code is valid and not expired
        if ($user['reset_token'] === null) {
            $error = "Verification failed: No reset token found";
        } elseif ($user['reset_expiry'] < $current_time) {
            $error = "Verification failed: Code has expired";
        } elseif ($code !== $user['reset_token']) {
            $error = "Verification failed: Invalid code";
        } else {
            // Code is valid - clear the reset token and proceed
            $update_sql = "UPDATE users SET reset_token = NULL, reset_expiry = NULL WHERE id = ?";
            $update_stmt = mysqli_prepare($conn, $update_sql);
            mysqli_stmt_bind_param($update_stmt, "i", $_SESSION['reset_user_id']);
            mysqli_stmt_execute($update_stmt);
            mysqli_stmt_close($update_stmt);
            
            // Redirect to reset password page
            header("location: reset_password.php");
            exit;
        }
        
        error_log("Verification result: " . (isset($error) ? $error : "Success"));
    }
}

// Debug: Display session info
error_log("Current session state:\n" .
          "User ID: " . $_SESSION['reset_user_id'] . "\n" .
          "Email: " . $_SESSION['reset_email'] . "\n" .
          "Code: " . $_SESSION['reset_code'] . "\n" .
          "Expiry: " . (isset($_SESSION['reset_expiry']) ? $_SESSION['reset_expiry'] : 'Not set'));
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Verify Code - Nehemiah Prestress Inventory Management System</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="assets/css/auth.css">
    <link rel="icon" type="image/jpg" href="images/nehemiahlogo.jpg">
    <style>
        body {
            min-height: 100vh;
            display: flex;
            align-items: center;
            justify-content: center;
            background: url('images/loginbg.jpg') no-repeat center center fixed;
            background-size: cover;
            font-family: 'Poppins', sans-serif;
            padding: 1rem;
            position: relative;
        }

        body::before {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
            right: 0;
            bottom: 0;
            background: rgba(0, 0, 0, 0.6);
            z-index: 1;
        }

        .auth-container {
            width: 100%;
            max-width: 420px;
            background: rgba(255, 255, 255, 0.95);
            border-radius: 16px;
            box-shadow: 0 8px 32px rgba(0, 0, 0, 0.2);
            overflow: hidden;
            position: relative;
            z-index: 2;
            backdrop-filter: blur(10px);
        }

        .auth-header {
            text-align: center;
            padding: 2rem;
            background: #fff;
            margin-bottom: 1rem;
        }

        .auth-header h2 {
            font-size: 1.75rem;
            font-weight: 600;
            margin-bottom: 0.5rem;
            color: #dc3545;
        }

        .auth-header p {
            margin: 0;
            font-size: 1rem;
            color: #6c757d;
        }

        .auth-body {
            padding: 2rem;
        }

        .alert {
            border: none;
            border-radius: 12px;
            padding: 1rem;
            margin-bottom: 1.5rem;
            font-size: 0.9rem;
            background: rgba(255, 255, 255, 0.9);
        }

        .alert-info {
            background-color: rgba(232, 244, 248, 0.9);
            color: #0c5460;
            border-left: 4px solid #17a2b8;
        }

        .alert-danger {
            background-color: rgba(253, 243, 244, 0.9);
            color: #dc3545;
            border-left: 4px solid #dc3545;
        }

        .form-label {
            font-weight: 500;
            color: #344767;
            margin-bottom: 0.5rem;
            font-size: 0.875rem;
        }

        .form-control {
            padding: 0.75rem 1rem;
            border-radius: 12px;
            border: 1px solid rgba(233, 236, 239, 0.8);
            font-size: 0.875rem;
            transition: all 0.2s ease;
            background: rgba(255, 255, 255, 0.9);
            text-align: center;
            letter-spacing: 0.5em;
        }

        .form-control:focus {
            border-color: #dc3545;
            box-shadow: 0 0 0 3px rgba(220, 53, 69, 0.15);
            background: white;
        }

        .btn-primary {
            background-color: #dc3545;
            border: none;
            padding: 0.75rem;
            border-radius: 12px;
            font-weight: 500;
            font-size: 0.875rem;
            transition: all 0.2s ease;
            width: 100%;
        }

        .btn-primary:hover {
            background-color: #c82333;
            transform: translateY(-1px);
            box-shadow: 0 4px 12px rgba(220, 53, 69, 0.2);
        }

        .auth-links {
            text-align: center;
            margin-top: 1.5rem;
            padding-top: 1.5rem;
            border-top: 1px solid rgba(233, 236, 239, 0.4);
        }

        .auth-links a {
            color: #dc3545;
            text-decoration: none;
            font-size: 0.875rem;
            font-weight: 500;
            transition: all 0.2s ease;
            padding: 0.5rem 1rem;
            border-radius: 8px;
            background: rgba(255, 255, 255, 0.1);
        }

        .auth-links a:hover {
            color: #c82333;
            background: rgba(255, 255, 255, 0.2);
        }

        .mb-3 {
            margin-bottom: 1.25rem !important;
        }

        @media (max-width: 576px) {
            .auth-container {
                margin: 1rem;
            }

            .auth-header {
                padding: 1.5rem;
            }

            .auth-body {
                padding: 1.5rem;
            }
        }
    </style>
</head>
<body>
    <div class="auth-container">
        <div class="auth-header">
            <h2>Nehemiah Prestress</h2>
            <p>Verify Code</p>
        </div>
        
        <div class="auth-body">
            <?php if(!empty($error)): ?>
                <div class="alert alert-danger">
                    <strong>Error!</strong> <?php echo $error; ?>
                </div>
            <?php endif; ?>

            <div class="alert alert-info">
                A verification code has been sent to <?php echo htmlspecialchars($_SESSION['reset_email']); ?>. 
                Please enter the 6-digit code below.
            </div>

            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div class="mb-3">
                    <label for="code" class="form-label">Verification Code</label>
                    <input type="text" class="form-control" id="code" name="code" 
                           placeholder="Enter 6-digit code" maxlength="6" required>
                </div>
                <div class="d-grid">
                    <button type="submit" name="verify" class="btn btn-primary">
                        Verify Code
                    </button>
                </div>
            </form>
            
            <div class="auth-links">
                <a href="forgot_password.php">← Back to Forgot Password</a>
            </div>
        </div>
    </div>
    
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script>
    <script>
        // Auto-focus the code input
        document.getElementById('code').focus();
        
        // Allow only numbers
        document.getElementById('code').addEventListener('input', function(e) {
            this.value = this.value.replace(/[^0-9]/g, '');
        });
    </script>
</body>
</html> 